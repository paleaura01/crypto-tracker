...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const srcFilePath = join(process.cwd(), 'src', 'data', 'overrides', 'address-overrides.json');
    const staticFilePath = join(process.cwd(), 'static', 'data', 'address-overrides.json');
    ...
    return json({ success: true, message: 'Address overrides saved successfully' });  } catch (error: unknown) {
